    ...
    try {
      ...
      data_category = await page.evaluate(
        (listPage, numberPage) => {
          ...
          let elementH2 = document.querySelectorAll(".box.clearfix.font-opensans-b > h2");
          let element = [...elementDiv];
          ...
          element.forEach((item, key) => {
            let data_item = {
              name: listPage[numberPage].name + (elementH2[key] && elementH2[key].textContent ? " - " + elementH2[key].textContent.trim() : ""),
              title: listPage[numberPage].name + (elementH2[key] && elementH2[key].textContent ? " - " + elementH2[key].textContent.trim() : ""),
            ...
            //loai 1
            let element_child_1 = item.querySelectorAll("div.subject-item");
            element_child_1.forEach((item_c1, key_c1) => {
              let temp_child_1 = {
                name: item_c1.querySelector("h2 > a > strong").textContent.trim(),
                title: item_c1.querySelector("h2 > a > strong").textContent.trim(),
                parent_title: data_item.title,
                class: listPage[numberPage].class,
                category: listPage[numberPage].category,
                book: listPage[numberPage].book,
                url_crawl: "",
              ...
              data_item.child.push(temp_child_1);
            });

            //loai 2
            if (data_item.child.length == 0) {
              let element_child_1 = item.querySelectorAll("ul.clearfix.font-16 > li");
              element_child_1.forEach((item_c1, key_c1) => {
                let temp_child_1 = {
  ...
  try {
    await axios
      ...
      .catch(async function (error) {
        ...
        await sendTele(error, data, "Error Saved Database");
      });
  } catch (error) {
    console.log("LOCAL LOG: ERROR SAVE DATABASE");
  }
